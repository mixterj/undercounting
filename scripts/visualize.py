...
if __name__ == '__main__':
    ...
    for line in sites:
        ...
        results = main(startDate, endDate, line, sys.argv)
        master_results.append(results)
    for items in master_results:
        for response in items:
            for item in response["rows"]:
               # print item
                if item["keys"][1] in hits:
                    hits[str(item["keys"][1])] = hits[str(item["keys"][1])] + 1
                else:
                    hits[str(item["keys"][1])] = 1
    apiData = {}
    apiData["cols"] = []
    apiData["cols"].append({'id': 't', 'label': 'Device', 'type': 'string'})
    apiData["cols"].append({'id': 't', 'label': 'Count', 'type': 'number'})
    apiData["rows"] = []
    for key, value in hits.iteritems():
      ...
      cDict = { 'c': vList }
      apiData["rows"].append(cDict)
    print json.dumps(apiData)
